  ...
  return (
    <Text style={[styles.paragraphText, { color, fontSize, lineHeight: fontSize * 1.66, fontFamily }]}> 
      {parts.map((part, index) => {
        ...
        if (part.startsWith('==') && part.endsWith('==')) {
          return <Text key={`inline-mark-${index}`} style={styles.mark}>{part.slice(2, -2)}</Text>;
        }
        if (part.startsWith('`') && part.endsWith('`')) {
          return <Text key={`inline-code-${index}`} style={styles.inlineCode}>{part.slice(1, -1)}</Text>;
        }
  ...
  return (
    <View style={styles.wrap}>
      {blocks.map((block, index) => {
        ...
        if (block.type === 'code') {
          return (
            <View key={`block-code-${index}`} style={styles.codeBox}>
        ...
        if (block.type === 'quote') {
          return (
            <View key={`block-quote-${index}`} style={styles.quote}>
              <Inline text={block.text} color={palette.inkMuted} fontSize={Math.max(15, fontSize - 1)} fontFamily={fontFamily} />
            </View>
          );
        }
        if (block.type === 'list') {
        ...
        if (block.type === 'table') {
          ...
          return (
            <View key={`block-table-${index}`} style={styles.tableFrame}>
              <ScrollView horizontal nestedScrollEnabled showsHorizontalScrollIndicator={columnCount > 3} style={styles.tableScroller} contentContainerStyle={styles.tableScrollerInner}>
                <View style={styles.table}>
                  {rows.map((row, rowIndex) => {
                    ...
                    return (
                      <View key={`row-${rowIndex}`} style={[styles.tableRow, !isLastRow && styles.tableRowDivider, isHeader && styles.tableHeaderRow]}>
                        {Array.from({ length: columnCount }).map((_, cellIndex) => {
                          const isLastCell = cellIndex === columnCount - 1;
                          return (
                            <View key={`cell-${rowIndex}-${cellIndex}`} style={[styles.tableCell, { width: columnWidth }, isHeader && styles.tableHeaderCell, !isLastCell && styles.tableCellDivider, !isHeader && rowIndex % 2 === 0 && styles.tableCellAlt]}>
                              <Text style={[isHeader ? styles.tableHeaderText : styles.tableCellText, { fontFamily }]}>{row[cellIndex] ?? ''}</Text>
                            </View>
                          );
                        })}
                      </View>
